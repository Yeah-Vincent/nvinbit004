...
def 将u设置为空():
    ...
    USERS_WORDS_ON_SCREEN = ["",
        ...
        "",
        ""]
def USERS_WORDS_ON_THE_SCREEN():
    global la, d
    la = [""]
    for i in USERS_WORDS_ON_SCREEN:
        la.append(i)
    for a in la:
        d = "" + d + a
    return d
...
def on_button_pressed_b():
    ...
    if 判断按钮第几遍 == 0:
        if USERS_WORDS_ON_THE_SCREEN().char_at(len(USERS_WORDS_ON_THE_SCREEN()) - 1) == "/":
            OLED12864_I2C.clear()
            OLED12864_I2C.show_string(0, 0, "vin:bit 0.0.1", 270)
            OLED12864_I2C.show_string(0, 1, USERS_WORDS_ON_SCREEN[0], 1)
            临时 = USERS_WORDS_ON_SCREEN[0].substr(len(USERS_WORDS_ON_THE_SCREEN()) - 4,
                len(USERS_WORDS_ON_SCREEN[0]) - 1)
            临时1 = USERS_WORDS_ON_SCREEN[0].substr(len(USERS_WORDS_ON_THE_SCREEN()) - 9,
                len(USERS_WORDS_ON_SCREEN[0]) - 1)
            临时2 = USERS_WORDS_ON_SCREEN[0].substr(len(USERS_WORDS_ON_THE_SCREEN()) - 7,
                len(USERS_WORDS_ON_SCREEN[0]) - 1)
            临时3 = USERS_WORDS_ON_SCREEN[0].substr(len(USERS_WORDS_ON_THE_SCREEN()) - 5,
                len(USERS_WORDS_ON_SCREEN[0]) - 1)
            临时3a = USERS_WORDS_ON_SCREEN[0].substr(len(USERS_WORDS_ON_THE_SCREEN()) - 5,
                len(USERS_WORDS_ON_SCREEN[0]) - 2)
            临时3b = USERS_WORDS_ON_SCREEN[0].substr(USERS_WORDS_ON_THE_SCREEN().index_of("s"), 4)
            临时4 = USERS_WORDS_ON_SCREEN[0].substr(len(USERS_WORDS_ON_THE_SCREEN()) - 6, 3)
            当前命令 = ""
            临时_特殊 = 0
            ...
            if 临时_特殊 == 0:
                ...
                datalogger.log(datalogger.create_cv(临时d,
                        USERS_WORDS_ON_THE_SCREEN().substr(0, len(USERS_WORDS_ON_THE_SCREEN()) - 5)))
                ...
                系统显示字儿(USERS_WORDS_ON_THE_SCREEN().substr(USERS_WORDS_ON_THE_SCREEN().index_of("w") + 2,
                        len(USERS_WORDS_ON_THE_SCREEN()) - 1 - (USERS_WORDS_ON_THE_SCREEN().index_of("w") + 2)))
                判断是否达顶 = 1
            elif 临时4 == "sl.":
                led.plot(parse_float(USERS_WORDS_ON_THE_SCREEN().char_at(len(USERS_WORDS_ON_THE_SCREEN()) - 3)),
                    parse_float("".char_at(len(USERS_WORDS_ON_THE_SCREEN()) - 2)))
            elif 临时3 == "help":
                OLED12864_I2C.clear()
                OLED12864_I2C.show_string(0, 0, "vinbit 0.0.1", 1)
        ...
        临时a = parse_float(USERS_WORDS_ON_THE_SCREEN().substr(USERS_WORDS_ON_THE_SCREEN().index_of(":") + 1,
                USERS_WORDS_ON_THE_SCREEN().index_of("&") - 1 - USERS_WORDS_ON_THE_SCREEN().index_of(":")))
        临时b = parse_float(USERS_WORDS_ON_THE_SCREEN().substr(USERS_WORDS_ON_THE_SCREEN().index_of("&") + 1,
                len(USERS_WORDS_ON_THE_SCREEN()) - (USERS_WORDS_ON_THE_SCREEN().index_of("&") + 1)))
        if 当前命令 == "add":
...
def 清除():
    ...
    for 值6 in USERS_WORDS_ON_SCREEN:
        if 值6.is_empty():
            ...
            至第几行有空 += 1
    if 至第几行有空 == 0:
        OLED12864_I2C.show_string(0, 循环次数 + 1, USERS_WORDS_ON_THE_SCREEN(), 1)
    else:
        循环次数 = 0
        if 至第几行有空 > 4:
            ...
            for index2 in range(至第几行有空):
                OLED12864_I2C.show_string(0, 循环次数 + 1, USERS_WORDS_ON_SCREEN[循环次数], 1)
                循环次数 += 1
def 获取Users列表中最后一项有字的():
    global 循环次数
    循环次数 = 0
    for index3 in range(len(USERS_WORDS_ON_SCREEN)):
        循环次数 = 循环次数 + 1
...
while 获取Users列表中最后一项有字的() == 29:
    basic.pause(100)
    OLED12864_I2C.show_string(0, 1, "Warning: Screen full", 1)
临时3b = "0"
判断按钮第几遍 = 0
_26个字母与符号 = ["a",
    "b",
