...
function USERS_WORDS_ON_THE_SCREEN(): string {
    
    la = [""]
    for (let i of USERS_WORDS_ON_SCREEN) {
        la.push(i)
    }
    for (let a of la) {
        d = "" + d + a
...
input.onButtonPressed(Button.B, function on_button_pressed_b() {
    ...
    if (判断按钮第几遍 == 0) {
        if (USERS_WORDS_ON_THE_SCREEN().charAt(USERS_WORDS_ON_THE_SCREEN().length - 1) == "/") {
            OLED12864_I2C.clear()
            OLED12864_I2C.showString(0, 0, "vin:bit 0.0.1", 270)
            OLED12864_I2C.showString(0, 1, USERS_WORDS_ON_SCREEN[0], 1)
            临时 = USERS_WORDS_ON_SCREEN[0].substr(USERS_WORDS_ON_THE_SCREEN().length - 4, USERS_WORDS_ON_SCREEN[0].length - 1)
            临时1 = USERS_WORDS_ON_SCREEN[0].substr(USERS_WORDS_ON_THE_SCREEN().length - 9, USERS_WORDS_ON_SCREEN[0].length - 1)
            临时2 = USERS_WORDS_ON_SCREEN[0].substr(USERS_WORDS_ON_THE_SCREEN().length - 7, USERS_WORDS_ON_SCREEN[0].length - 1)
            临时3 = USERS_WORDS_ON_SCREEN[0].substr(USERS_WORDS_ON_THE_SCREEN().length - 5, USERS_WORDS_ON_SCREEN[0].length - 1)
            临时3a = USERS_WORDS_ON_SCREEN[0].substr(USERS_WORDS_ON_THE_SCREEN().length - 5, USERS_WORDS_ON_SCREEN[0].length - 2)
            临时3b = USERS_WORDS_ON_SCREEN[0].substr(USERS_WORDS_ON_THE_SCREEN().indexOf("s"), 4)
            临时4 = USERS_WORDS_ON_SCREEN[0].substr(USERS_WORDS_ON_THE_SCREEN().length - 6, 3)
            当前命令 = ""
            临时_特殊 = 0
            ...
            if (临时_特殊 == 0) {
                ...
                datalogger.log(datalogger.createCV(临时d, USERS_WORDS_ON_THE_SCREEN().substr(0, USERS_WORDS_ON_THE_SCREEN().length - 5)))
                ...
                系统显示字儿(USERS_WORDS_ON_THE_SCREEN().substr(USERS_WORDS_ON_THE_SCREEN().indexOf("w") + 2, USERS_WORDS_ON_THE_SCREEN().length - 1 - (USERS_WORDS_ON_THE_SCREEN().indexOf("w") + 2)))
                判断是否达顶 = 1
            } else if (临时4 == "sl.") {
                led.plot(parseFloat(USERS_WORDS_ON_THE_SCREEN().charAt(USERS_WORDS_ON_THE_SCREEN().length - 3)), parseFloat("".charAt(USERS_WORDS_ON_THE_SCREEN().length - 2)))
            } else if (临时3 == "help") {
                OLED12864_I2C.clear()
                OLED12864_I2C.showString(0, 0, "vinbit 0.0.1", 1)
        ...
        临时a = parseFloat(USERS_WORDS_ON_THE_SCREEN().substr(USERS_WORDS_ON_THE_SCREEN().indexOf(":") + 1, USERS_WORDS_ON_THE_SCREEN().indexOf("&") - 1 - USERS_WORDS_ON_THE_SCREEN().indexOf(":")))
        临时b = parseFloat(USERS_WORDS_ON_THE_SCREEN().substr(USERS_WORDS_ON_THE_SCREEN().indexOf("&") + 1, USERS_WORDS_ON_THE_SCREEN().length - (USERS_WORDS_ON_THE_SCREEN().indexOf("&") + 1)))
        if (当前命令 == "add") {
...
function 清除() {
    ...
    if (至第几行有空 == 0) {
        OLED12864_I2C.showString(0, 循环次数 + 1, USERS_WORDS_ON_THE_SCREEN(), 1)
    } else {
        循环次数 = 0
        if (至第几行有空 > 4) {
...
function 获取Users列表中最后一项有字的(): number {
    
    循环次数 = 0
    for (let index3 = 0; index3 < USERS_WORDS_ON_SCREEN.length; index3++) {
        循环次数 = 循环次数 + 1
        if (USERS_WORDS_ON_SCREEN[循环次数] == "") {
            break
        } else {
            continue
        }
...
let 判断按钮第几遍 = 0
let 临时3b = ""
while (获取Users列表中最后一项有字的() == 29) {
    basic.pause(100)
    OLED12864_I2C.showString(0, 1, "Warning: Screen full", 1)
}
临时3b = "0"
判断按钮第几遍 = 0
